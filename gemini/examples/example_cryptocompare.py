...
days_data = 30  # From there collect X days of data
...
def logic(account, lookback, lookback_period):
    try:
        ...
        print('Look back from {} to {}'.format(yesterday['date'], today['date']))

        if today['close'] < yesterday['close']:
            exit_price = today['close']
            for position in account.positions:
                if position.type == 'Long':
                    print("{} Sell {}BTC @ ${} = ${} balance".format(today['date'], position.shares,
                                                                     exit_price,
        ...
        if today['close'] > yesterday['close']:
            ...
            if entry_capital > 0:
                ...
                print("{} Buy ${} of BTC @ ${} = {}BTC balance".format(today['date'], entry_capital,
                                                                       entry_price,
...
r.results()
r.chart('Look back Period: {}, Trading Interval: {}'.format(lookback_period, trading_interval),
        show_trades=True)
